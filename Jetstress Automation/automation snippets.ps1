...
Invoke-Command -ThrottleLimit 32 -ComputerName $computernames-scriptblock {param($admincreds) New-PSDrive -Name X -PSProvider FileSystem -Root \\Ex1\g$\js -Credential $admincreds; Copy-Item -Path x:\ -Destination "g:\" -Recurse} -ArgumentList $admincreds
#1..2 | foreach {"EX$_"} | Get-ADComputer | % {Invoke-Command -ComputerName $_.name {get-disk | ft}}
$computernames=(1..2 | foreach {"EX$($_)"} | Get-ADComputer).name
...
Invoke-Command -ThrottleLimit 32 -ComputerName EX2 -scriptblock {param($admincreds) New-PSDrive -Name X -PSProvider FileSystem -Root \\EX1\f$\Automation -Credential $admincreds; Copy-Item -Path x:\ -Destination "f:\" -Recurse} -ArgumentList $admincreds

Invoke-Command -ThrottleLimit 32 -ComputerName $computernames -scriptblock {param($admincreds) New-PSDrive -Name X -PSProvider FileSystem -Root \\ex1\g$\js -Credential $admincreds; Copy-Item -Path x:\ -Destination "g:\" -Recurse} -ArgumentList $admincreds

## running diskpart.ps1 on all the machines to set / reset mountpoints and volumes -- requires DBmap.csv
Invoke-Command -ThrottleLimit 32 -ComputerName $computernames -scriptblock {param($admincreds) f:\automation\diskpart.ps1 -serverfile f:\automation\labdag_servers.csv -whatif:$false} -ArgumentList $admincreds

Invoke-Command -ComputerName EX2 -ScriptBlock { 
    param($admincreds) Install-package -name "F:\Automation\Jetstress.msi" -Force
...
$computernames_contoso_DAG1 = 11..18 | foreach {"Ex$($_)SITE1"} 
$computernames_contoso_DAG1 += 19..26 | foreach {"Ex$($_)SITE2"} 
...
Invoke-Command -ComputerName EX2 -ScriptBlock { 
    param($admincreds) Uninstall-package -name "Microsoft Exchange Jetstress 2013" -Force
} -ArgumentList $admincreds
